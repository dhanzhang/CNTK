    ...
    const int pooled_height, const int pooled_width, const Matrix<ElemType>& roi_data, Matrix<ElemType>& output, Matrix<ElemType>& argmax) const
{
    ...
    DISPATCH_MATRIX_ON_FLAG(this,
        this,
        m_CPUMatrix->ROIPoolingForward(num_rois, img_count, channels, height, width, pooled_height, pooled_width, *(roi_data.m_CPUMatrix), *(output.m_CPUMatrix), *(argmax.m_CPUMatrix)),
        m_GPUMatrix->ROIPoolingForward(num_rois, img_count, channels, height, width, pooled_height, pooled_width, *(roi_data.m_GPUMatrix), *(output.m_GPUMatrix), *(argmax.m_GPUMatrix)),
        NOT_IMPLEMENTED,
        NOT_IMPLEMENTED);
}

template <class ElemType>
void Matrix<ElemType>::ROIPoolingBackward(const int num_rois, const int img_count, const int channels, const int height, const int width,
    const int pooled_height, const int pooled_width, const Matrix<ElemType>& roi_data, Matrix<ElemType>& grad, Matrix<ElemType>& argmax) const
{
    ...
    DISPATCH_MATRIX_ON_FLAG(this,
        this,
        m_CPUMatrix->ROIPoolingBackward(num_rois, img_count, channels, height, width, pooled_height, pooled_width, *(roi_data.m_CPUMatrix), *(grad.m_CPUMatrix), *(argmax.m_CPUMatrix)),
        m_GPUMatrix->ROIPoolingBackward(num_rois, img_count, channels, height, width, pooled_height, pooled_width, *(roi_data.m_GPUMatrix), *(grad.m_GPUMatrix), *(argmax.m_GPUMatrix)),
        NOT_IMPLEMENTED,
        NOT_IMPLEMENTED);
}
